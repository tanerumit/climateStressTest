...
createSurfacePlot <- function(
...

  {
    ...
    gg_theme_surface <- function(font = "Arial", size = 18) {
      theme_minimal() %+replace%
      theme(
        axis.title = element_text(family = font, size = size),
        legend.title = element_text(family = font, size = size),
        axis.text = element_text(family = font, size = size-2),
        legend.text = element_text(family = font, size = size-2)
      )
    }
    ...
    z_min  <- min(str.data[[variable.z]]) %>% floor()
    z_max  <- max(str.data[[variable.z]]) %>% ceiling()
    ...
    z_breaks_legend <- pretty(c(z_min_legend, z_max_legend), z_bin)
    ...
    p <- ggplot(str.data, aes(x = .data[[variable.x]], y = .data[[variable.y]])) +
      ...
      # Place z dimension
      geom_contour_filled(aes(z = .data[[variable.z]], fill = stat(level_mid)),
        breaks = z_breaks) +
      geom_contour(aes(z = .data[[variable.z]]), breaks = threshold.z,
      ...
      scale_fill_divergent(low = color.low, mid = "white",
         ...
         limits = range(z_breaks_legend), breaks = z_breaks_legend,
              #guide_bins(order = 1, axis.linewidth = 1, axis.color = "black")) +
      ...
      labs(x = variable.x.label,y = variable.y.label,
        color = NULL, fill = variable.z.label, title = plot.title)

      rcp_col <- c("rcp26" = "#0000FF", "rcp45" = "#79BCFF",
          "rcp60" = "#FF822D", "rcp85" = "#FF0000")

      rcp_labels <- c("RCP2.6", "RCP4.5", "RCP6.0", "RCP8.5")
      ...
      if(!is.null(gcm.data)) {
        ...
        #sind <- which(names(rcp_col) %in% scenarios_list)
